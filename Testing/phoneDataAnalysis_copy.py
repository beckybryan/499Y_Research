...
def main():
	...
	#If user manually enters a certain date
	if(len(sys.argv[2]) == 7 and len(sys.argv[3]) == 7):
		...
		sys.exit()
	elif(sys.argv[2] == "a" and sys.argv[3]== "a"):
		#Tests and trains on all data
		users_dict = read_data(fd)
		train_test_dates = [
			"2003-12",
			"2004-01",
	...
	#This is where things can change... how to make multiple processes? OR DO I EVEN NEED MULTIPPROCESSES? perhaps it won't make much of a difference.
	#users_dict = train_all_data(users_dict, train_year_month)
...
""" NOTE USED: Asks for a date range input (note: date1 and date2 will be in the form '2004-07-26 00:00:00') and number of users to test.
...
def test_data_old(trained_users):
	correct=0
	while(not correct):
		print("- - - - - - - - - - - - - - - - - - - - - - - - -")
